...
| `DIFFSHUB_REQUIRE_LOGIN`                                      | unset                                                      | Set to `1`/`true` to require credentials on every page: visitors without a saved token are redirected to `/login` (GitHub sign-in or PAT) and returned to their original URL afterward. |
