...
describe('commitRangeViewerPath', () => {
  test('a single commit opens its own commit diff', () => {
    expect(commitRangeViewerPath(REPO, COMMITS[1], COMMITS[1])).toBe(
      '/octo/demo/commit/c2'
    );
  });

  test('a range compares from the start commit parent to the end', () => {
    expect(commitRangeViewerPath(REPO, COMMITS[1], COMMITS[3])).toBe(
      '/octo/demo/compare/c1...c4'
    );
  });

  test('a range starting at the first commit spans from the pull base', () => {
    expect(commitRangeViewerPath(REPO, COMMITS[0], COMMITS[2])).toBe(
      '/octo/demo/compare/base0...c3'
    );
  });

  test('a parentless start returns null', () => {
    expect(
