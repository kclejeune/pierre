...
| `DIFFSHUB_TOKEN_ENCRYPTION_KEY`                               | unset (tokens stored in the clear)                         | Opt-in at-rest encryption of the credentials the browser stores: 32 base64-encoded bytes, e.g. `openssl rand -base64 32`. When set, OAuth access and refresh tokens leave the server sealed into AES-256-GCM envelopes, so localStorage (and its backups) holds only ciphertext GitHub never accepts directly; the API routes decrypt on arrival. Rotating or removing the key signs every sealed session out. Bare tokens — pasted PATs and sessions predating the key — keep working unencrypted.                                                                                                                                                                                                          |
| `DIFFSHUB_REQUIRE_SEALED_TOKENS`                              | off                                                        | Accept only server-sealed credentials at the API routes (needs `DIFFSHUB_TOKEN_ENCRYPTION_KEY` and GitHub OAuth configured, since only the OAuth grant flow mints sealed credentials). An envelope can only come from this deployment's own OAuth flow, so this is the server-enforced counterpart of hiding the PAT box: pasted PATs, tokens minted elsewhere, and sessions predating the encryption key all stop working and must re-authenticate through GitHub.                                                                                                                                                                                                                                          |
