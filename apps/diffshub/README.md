...
| `DIFFSHUB_REQUIRE_LOGIN`                                      | unset                                                      | Set to `1`/`true` to require credentials: visitors without a saved token are redirected to `/login` (GitHub sign-in or PAT) and returned to their original URL afterward, and the API routes refuse tokenless requests — the server fallback token is never used on behalf of anonymous callers in this mode. |
...
avatars and pasted user-attachment images on private-mode GHES). Signed-in
viewers load all of these with their own token; anonymous visitors need the
