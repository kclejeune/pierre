...
| `DIFFSHUB_ENABLE_PAT_INPUT`                                   | off when login is required and OAuth is configured         | Whether the UI offers the manual PAT paste box and token-creation links. `0`/`false` hides them everywhere, leaving "Sign in with GitHub" as the only offered auth method; `1`/`true` forces them on. When unset, a require-login deployment with OAuth configured hides the box (PATs stay out of localStorage); without OAuth it stays on, since a PAT is then the only way through the login gate. UI-only; the server does not distinguish token origins.                                                                                                                                                    |
| `DIFFSHUB_REFRESH_TOKEN_MAX_TTL`                              | unset (GitHub's six months)                                | Caps how long a refresh token stored in the browser stays usable, e.g. `86400`, `24h`, or `7d`. Applied to every grant, so an actively refreshing browser keeps sliding forward but one idle past the cap must sign in again; a positive cap is enforced by the client at the capped expiry (the server cannot tell a capped token from a fresh one). `0` is server-enforced: refresh tokens are omitted from every grant and the refresh route refuses exchanges, forcing a full reauthorization once the short-lived (8h) access token expires. Only meaningful for GitHub Apps with token expiration enabled. |
