...
| `DIFFSHUB_AVATAR_TOKEN`                                       | unset (avatars use the viewer's token)                     | Classic GitHub token used for nothing but GHES avatar lookups. GHES gates its avatar API on a classic OAuth scope (`read:user` is enough) and exposes no equivalent GitHub App permission, so a viewer signed in through a GitHub App cannot load avatars with their own token. Fine-grained PATs use the permission model and do not work either. Prefer a dedicated machine account, since classic scopes are bounded by what that account can already see. This setting requires `DIFFSHUB_TOKEN_ENCRYPTION_KEY`; DiffsHub only lends the shared credential to a verified, server-sealed session. Unset, blank, or rejected by the instance, avatars fall back to the viewer's token and then to initials. |
...
serve avatars to a GitHub App token at all. DiffsHub verifies the caller's
sealed credential before using the shared token. Successful avatar responses
carry a one-hour private browser-cache lifetime; responses fetched with a
viewer's token use the standard five-minute lifetime. `Vary: Authorization`
keeps both kinds scoped to the credential that requested them.

No Caddy cache configuration is needed. Standard Caddy `reverse_proxy` passes
these headers through, and the browser performs the caching after DiffsHub has
...
briefly; hydrated diff files and recursive repository trees are cached for 30
minutes with entry caps and an approximate 32 MiB payload budget for each
large-object cache. Credentials are represented only by SHA-256 digests in cache
keys. Once a branch or tag resolves, the payload key uses the full commit SHA,
so different refs pointing to the same revision share the expensive result
without sharing data across credentials.

`/api/diff` deliberately remains uncached. Its response can exceed 100 MiB and
is streamed progressively; retaining it would require buffering the whole
payload and could multiply process memory use.
...
```text
{"time":"...","event":"api_request","method":"GET","path":"/api/github-web-asset","handlerDurationMs":26,"level":"info","status":200}
```

`level` is `info` below 400, `warn` for a 4xx, and `error` for a 5xx. A request
whose handler throws also carries `error` and `stack`. `handlerDurationMs` stops
when the route returns its response; streamed body transfer may continue.
`/api/health` is the one route not logged because it is polled continuously.
...
login gate, for ECS/ALB target groups, App Runner, and similar probes. Invalid
static configuration returns 503 so the instance does not enter service. Do not
...
avatars and pasted user-attachment images on private-mode GHES). Repository and
attachment images load with the viewer's token. Avatar API lookups may use the
configured deployment credential after the viewer session is verified; for
anonymous visitors on github.com only public-repository assets render.
